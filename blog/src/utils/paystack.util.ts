...
export const updateSubaccount = async (data: object | any): Promise<any> => {
    ...
    const build = {
        ...
        percentage_charge: data.perCharge,
        active: true,
        description: 'Checkamm subaccount for ' + data.businessName,
    ...
    await Axios.put(`${process.env.PAYSTACK_API_URL}/subaccount/${data.subaccountCode}`, {...build} , config)
    .then((resp) => {
        result = resp.data;
    }).catch((err) => {
        result = err.response.data;
...
export const getSubaccount = async (data: object | any): Promise<any> => {

    let result;

    await Axios.get(`${process.env.PAYSTACK_API_URL}/subaccount/${data.subaccountCode}`, config)
    .then((resp) => {
        result = resp.data;
    }).catch((err) => {
        result = err.response.data;
