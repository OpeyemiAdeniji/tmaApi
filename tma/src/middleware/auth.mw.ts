...
export const protect = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if(authCheck === null){
            return next(new ErrorResponse('Invalid token', 401, ['user not authorized to access this route a']))
        }

        req.user = {_id: authCheck.id || '', email: authCheck.email || '', roles: authCheck.roles || []};

        if(!authCheck.id || !authCheck.roles){
			return next(new ErrorResponse(`Unauthorized!`, 401, ['User is not authorized to access this route b']));
		}else{
			return next();
		}
        ...
        // console.log(err);
        return next(new ErrorResponse('Error!', 401, ['user not authorized to access this route d']))
...
export const authorize = (roles: Array<string>) => {
    ...
    return asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
        ...
        if(!authPermit){
            return next (new ErrorResponse('unauthorized!', 401, ['user is not authorized to access this route c']))
        }else{
            return next();
        }
